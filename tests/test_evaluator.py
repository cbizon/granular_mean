...
def test_brunner_treats_invalid_trajectory_as_candidate_failure(
    tmp_path,
    collection_factory,
    monkeypatch,
) -> None:
    ...
    contract = load_output_contract(definition.contract_path)
    reference_source = collection_factory(
        "failed-reference-source",
        language="updated-c",
    )
    reference_root = tmp_path / "failed-reference"
    generated = reference_root / "generated"
    shutil.copytree(reference_source.parent, generated)
    build_reference_manifest(
        reference_root,
    ...
    definition = replace(
        definition,
        reference=replace(definition.reference, root=reference_root),
    )
    trial = create_trial(
        definition,
        contract,
    ...
    spec = replace(
        ...
        command=(sys.executable, "-m", "granular_mean.evaluator"),
        reference_validate_command=(
            sys.executable,
            "-m",
            "granular_mean.reference_validation",
    ...
    result = execute_evaluation(spec, trial, reference_root=reference_root)

    assert result["status"] == "failed"
    assert result["failure"]["domain"] == "candidate"
    assert result["summary"]["cases_failed"] == ["a"]
